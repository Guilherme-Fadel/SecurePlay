...
export class S3Service {
  ...
  constructor(private configService: ConfigService) {
    this.bucketName = this.configService.get<string>('S3_BUCKET_NAME', 'secureplay-media');
  ...
  buildKey(type: 'thumbnail' | 'video' | 'page', moduloId: number, aulaId?: number, pageOrder?: number): string {
    switch (type) {
      case 'thumbnail':
