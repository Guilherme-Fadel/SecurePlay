...
/**
 * Deriva variáveis de cores da marca (primary/secondary/accent + variantes).
 * NÃO sobrescreve background/surface/border/text — esses continuam sendo
 * controlados pelo ThemeProvider (data-theme dark/light) via theme.css.
 */
function buildBrandVars(paleta: {
  primary: string;
  secondary: string;
  accent: string;
...
/**
 * Aplica a paleta da empresa do usuário logado como CSS custom properties.
 * Só sobrescreve cores da marca (primary/secondary/accent + variantes).
 * Background, surface, text e o comportamento dark/light continuam gerenciados
 * pelo ThemeProvider + theme.css normalmente.
 */
export function useEmpresaTema() {
  ...
  useEffect(() => {
    ...
    // Só aplica se a paleta existe e tem cores hex válidas
    if (!paleta || !isValidHex(paleta.primary)) return;
