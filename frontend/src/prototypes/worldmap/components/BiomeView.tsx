...
export function BiomeView({ biome, onSelectLevel, onBack, editMode, onReposition, }: BiomeViewProps) {
    const levels = biome.levels;
    return (<motion.div className="wm-screen" initial={{ opacity: 0, scale: 1.08 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.08 }} transition={{ duration: 0.4, ease: 'easeInOut' }}>
      ...
      <div className="wm-global-wrap wm-biome-wrap">
        ...
        {levels.map((level) => (<LevelNode key={level.id} level={level} onClick={onSelectLevel} editMode={editMode} onReposition={onReposition}/>))}
