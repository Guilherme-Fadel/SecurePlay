...
export function AchievementIcon({ icon, size = 28 }: { icon: string; size?: number }) {
  ...
  if (/^(https?:\/\/|\/)/.test(icon) && failedSource !== icon) {
    return <img className="achievement-artwork" src={icon} alt="" aria-hidden="true" width={size} height={size} style={{ width: size, height: size, objectFit: 'contain', imageRendering: 'auto' }} onError={() => setFailedSource(icon)} />;
  }
