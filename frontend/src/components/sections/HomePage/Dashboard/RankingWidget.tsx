...
export function RankingWidget() {
  ...
  const podium = ranking.top.length >= 3
    ? [ranking.top[1], ranking.top[0], ranking.top[2]]
    : ranking.top.length === 2
      ? [ranking.top[1], ranking.top[0]]
      : ranking.top;
  ...
  return (
    <InfoCard variant="accent" raised className="dashboard-ranking-card flex flex-col h-full min-h-0 overflow-hidden">
      <div className="dashboard-ranking-body">
        <div className={`academy-ranking-podium count-${podium.length}`}>
        ...
        <div className="academy-ranking-you">
          ...
          <span><small>SUA POSIÇÃO</small><strong>{currentUser.name}</strong></span>
          <b>{currentUser.points.toLocaleString('pt-BR')} XP</b>
        </div>
      </div>
...
function PodiumPlace({ entry }: { entry: RankingEntry }) {
  return (
    <div className={`academy-podium-place rank-${entry.position}`}>
      {entry.position === 1 && <Crown className="academy-podium-crown" size={17} aria-hidden="true" />}
